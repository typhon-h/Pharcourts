<div class="footer column">
  <a href="#body"><img src="./media/return-to-top.png" alt="Return to Top"></a> <!-- Up arrow ^ of some kind -->

  <div class="footer-content row">
    <div class="footer-section column">
      <!-- Newsletter Subscription -->
      <div class="newsletter">
        <h4>Subscribe to Our Newsletter</h4>
        <form class="subscribe-form row" method="post">
          <input type="text" name="subscribe-email" placeholder="Email Address">
          <input type="submit" name="subscribe-button" value="Subscribe">
        </form>
      </div>

    <!-- Social Media Links -->
    <div class="media-links row">
      <a href="https://www.facebook.com/PharcourtsTimaru/" target="_blank"><img src="./media/social/facebook-icon.png" alt="Facebook"></a>
      <a href="https://twitter.com/pharcourtsnz?lang=en" target="_blank"><img src="./media/social/twitter-icon.png" alt="Twitter"></a>
      <a href="https://www.youtube.com/watch?v=dQw4w9WgXcQ" target="_blank"><img src="./media/social/youtube-icon.png" alt="YouTube"></a>
      <a href="https://www.instagram.com/Pharcourts/" target="_blank"><img src="./media/social/instagram-icon.png" alt="Instagram"></a>
    </div>

    </div>

  <!-- Customer Quotes/Testimonials -->
    <div class="footer-section column testimonials">
      <div class="slider-testimonial">
        <div class="testimonial column">
          <p>"Lorem. Ipsum. Dolor."</p>
          <i>- A Happy Customer</i>
        </div>

        <div class="testimonial column">
          <p>"Lorem ipsum dolor sit amet, consectetur adipiscing elit."</p>
          <i>- A Sastisfied Customer</i>
        </div>

        <div class="testimonial column">
          <p>
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit.
            Suspendisse a vulputate ex, quis tristique lectus."
          </p>
          <i>- A Professional Reviewer</i>
        </div>

    </div>
</div>
    <!-- Quality of Life/Legal -->
    <div class="footer-section column legal-information">
      <h5><a href="#">Privacy Policy</a></h5>
      <h5><a href="#">Terms and Conditions</a></h5>
      <h5><a href="#">Feedback</a></h5>
    </div>
  </div>
</div>
